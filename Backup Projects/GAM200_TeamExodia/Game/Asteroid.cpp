...
void Asteroid::ResolveCollision(GameObject* other_object)
{
    ...
    {
        SetPosition({ GetPosition().x, other_object->GetGOComponent<CS230::RectCollision>()->WorldBoundary().Top() + 1}); //Added extra pixel of height
...
void Asteroid::State_Landing::Enter(GameObject* object)
{
    ...
    asteroid->GetGOComponent<CS230::Sprite>()->PlayAnimation(static_cast<int>(Animations::AsteroidLand));
