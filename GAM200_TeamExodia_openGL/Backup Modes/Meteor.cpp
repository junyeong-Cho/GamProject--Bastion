...
Meteor::Meteor(Meteor* parent) : GAM200::GameObject({ 0, 0 })
{
    AddGOComponent(new GAM200::Sprite("assets/Meteor.spt", this));
    ...
    if (parent == nullptr)
    {
        ...
        int boundary = GetGOComponent<GAM200::Sprite>()->GetFrameSize().x; // Assuming x and y size are the same
...
void Meteor::ResolveCollision(GameObject* other_object)
{
    ...
    case GameObjectTypes::Laser:
        ...
        particlePosition.x += vectorToCollision.x * GetGOComponent<GAM200::Sprite>()->GetFrameSize().x / 2;
        particlePosition.y += vectorToCollision.y * GetGOComponent<GAM200::Sprite>()->GetFrameSize().y / 2;


        Engine::GetGameStateManager().GetGSComponent<GAM200::ParticleManager<Particles::Hit>>()->Emit(1, particlePosition, { 0, 0 }, { 0, 100 }, Math::PI / 2);
        Engine::GetGameStateManager().GetGSComponent<GAM200::ParticleManager<Particles::MeteorBit>>()->Emit(5, particlePosition, { 0, 0 }, { 0, 100 }, Math::PI / 3);
    ...
    if (health < 0)
    {
        ...
        GetGOComponent<GAM200::Sprite>()->PlayAnimation(static_cast<int>(Animations::Fade));
        ...
        if (size > 0)
        {
            ...
            Engine::GetGameStateManager().GetGSComponent<GAM200::GameObjectManager>()->Add(meteor_1);
            Engine::GetGameStateManager().GetGSComponent<GAM200::GameObjectManager>()->Add(meteor_2);
        }
    }
}
...
void Meteor::Update(double dt)
{
    ...
    if (GetGOComponent<GAM200::Sprite>()->CurrentAnimation() == static_cast<int>(Animations::Fade) && GetGOComponent<GAM200::Sprite>()->AnimationEnded())
    {
        Destroy();
    }

    if (GetPosition().x > Engine::GetWindow().GetSize().x + GetGOComponent<GAM200::Sprite>()->GetFrameSize().x / static_cast<double>(2))
    {
        SetPosition({ 0 - GetGOComponent<GAM200::Sprite>()->GetFrameSize().x / static_cast<double>(2), GetPosition().y });
    }
    if (GetPosition().x < 0 - GetGOComponent<GAM200::Sprite>()->GetFrameSize().x / static_cast<double>(2))
    {
        SetPosition({ Engine::GetWindow().GetSize().x + GetGOComponent<GAM200::Sprite>()->GetFrameSize().x / static_cast<double>(2),0 });
    }

    if (GetPosition().y > Engine::GetWindow().GetSize().y + GetGOComponent<GAM200::Sprite>()->GetFrameSize().y / static_cast<double>(2))
    {
        SetPosition({ GetPosition().x,0 - GetGOComponent<GAM200::Sprite>()->GetFrameSize().y / static_cast<double>(2) });
    }
    if (GetPosition().y < 0 - GetGOComponent<GAM200::Sprite>()->GetFrameSize().y / static_cast<double>(2))
    {
        SetPosition({ GetPosition().x, Engine::GetWindow().GetSize().y + GetGOComponent<GAM200::Sprite>()->GetFrameSize().y / static_cast<double>(2) });
    }
