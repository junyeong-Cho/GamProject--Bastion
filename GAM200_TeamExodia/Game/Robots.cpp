...
Robot::Robot(Math::vec2 position, Cat* cat, double left_boundary, double right_boundary) : CS230::GameObject(position), m_cat(cat), m_left_boundary(left_boundary), m_right_boundary(right_boundary)
{
    AddGOComponent(new CS230::Sprite("Assets/Robot.spt", (this)));
    GetGOComponent<CS230::Sprite>()->PlayAnimation(static_cast<int>(Animations::None));
